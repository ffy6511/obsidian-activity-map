...
- 位于指标选择器左侧的“定位当前文件”按钮；点击后短暂高亮（约 2 秒）当前页眉所属文件对应的切片与图例行，等价于同时 hover 切片与图例。图例行不可见时以固定时长（约 400ms）的加速减速动画滚动到该行并尽量居中，目标位于列表边界附近时贴边不溢出；目标已在可视区内时不滚动。按路径分组模式下若文件不在当前范围，先将路径切换到文件所在最近一级目录再高亮。该高亮为临时状态，不打开、选中或固定该文件；
