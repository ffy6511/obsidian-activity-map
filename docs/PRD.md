...
hover 或键盘 focus 页眉按钮时打开环形图浮层；点击页眉按钮固定或取消固定该浮层。浮层右上角的“展开”按钮、Ribbon 图标或命令负责打开或聚焦完整 Activity Map 视图。移动端依赖点击、Ribbon 和命令，不要求 hover。

页眉集成优先使用 Obsidian 公开 API。公开接口无法覆盖时，DOM 适配必须隔离、可检测失败，并保留 Ribbon 与命令作为稳定降级入口。

### 页眉微型环形图

页眉入口保持同一个稳定的环形外框，不能随 tracking snapshot 在多个图标之间反复替换或闪烁。内圈使用当天真实数据表示“当前文件活动时间 / vault 活动时间”；尚无数据、查询失败或初始化期间只显示固定空环，不绘制伪造比例。

微型环形图只在当天汇总或当前文件累计值实际改变时更新几何，不因每个 runtime snapshot 重建 DOM。活动、空闲、待处理、暂停、不可追踪和存储异常通过稳定的可访问名称、tooltip、CSS 状态和必要的非闪烁语义标记表达，不能替换主图形。

### 环形图浮层

浮层是非模态的主要统计入口，默认显示今天的 vault 根目录，并与完整视图共享查询和图表语义。主体包含：

- Webtime Tracker 参考形态的交互式环形图；
- 指定日、前后日期、日均和全部历史范围控制；
- 当前路径面包屑、当前范围总量和 vault 占比；
- 与切片颜色严格对应的名称、百分比和精确时长图例；
- 独立的“展开完整视图”入口。

hover 或键盘 focus 切片时，同步高亮切片与图例，并显示名称、百分比和精确时长。点击目录切片在浮层内进入下一级；“本层文件”“其他”“已删除”和文件条目遵循完整视图的同一套激活规则。日期或范围切换保留当前路径。

指针或焦点进入浮层后保持打开；点击页眉按钮切换固定状态。未固定浮层在指针与焦点离开后延迟关闭；固定浮层只在再次点击页眉按钮、`Escape` 或外部点击时关闭。当前 tracking 状态、恢复决策和暂停操作可以作为紧凑辅助区出现，但不能取代环形图主体。全部信息和操作必须有键盘与完整视图等价入口。
...
页眉浮层与完整视图使用同一个查询状态、`ChartModel`、切片颜色、tooltip、图例和激活规则。“展开完整视图”继承浮层当前的范围、路径和指标，不能回到无关的默认查询。
