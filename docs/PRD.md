...
浮层不显示标题栏、重复的指标/总量/vault 占比汇总、图表下方的临时 tooltip 行或普通 tracking 状态页脚。hover 或键盘 focus 切片时只同步高亮切片与现有图例行，不能增删布局节点。点击目录切片在浮层内进入下一级；“本层文件”“其他”“已删除”和文件条目遵循完整视图的同一套激活规则。日期或范围切换保留当前路径。指标选择器可以使用随指标变化的语义图标。
