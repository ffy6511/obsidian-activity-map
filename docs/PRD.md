...
- 位于指标选择器左侧的“定位当前文件”按钮；目标始终是 Obsidian 当前激活的工作区文件，即使固定浮层来自先前页面也不会读取旧页眉路径。静止时显示 `locate`，成功定位后与切片、图例行的短暂高亮（约 2 秒）同时平滑过渡为 `locate-fixed`，高亮清除时同时恢复。该高亮等价于同时 hover 切片与图例。图例行不可见时以固定时长（约 400ms）的加速减速动画滚动到该行并尽量居中，目标位于列表边界附近时贴边不溢出；目标已在可视区内时不滚动。按路径分组模式下若文件不在当前范围，先将路径切换到文件所在最近一级目录再高亮。该高亮为临时状态，不打开、选中或固定该文件；
