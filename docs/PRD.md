...
- 首次使用时，tracking、路径/文件分组和导出海报位于左侧，Locate、指标和日期范围位于右侧。用户可仅在动作所属的一侧排序，或将动作移至可恢复的停用区；布局不会改写任何动作的语义；
...
在两个入口中，拖拽或键盘移动都只改变本地草稿：动作可在自身左右区域内排序，或拖到下方停用区。停用区中的动作仍可拖回它原本所属的一侧，不能跨过中心日期导航或进入另一侧。键盘操作提供等价的开始移动、同侧调整、停用、恢复和状态播报。

可拖拽的动作静止时显示 `grab` 光标；按下后切换为 `grabbing`，并将包含原图标或文字的按钮视觉副本随指针移动，产生直接将按钮拿起的效果。行内显示与按钮同尺寸的虚线圆角槽位：初始表示被拖起的位置，移动后表示当前可落位置。指针经过同侧两个动作之间时，槽位会插入对应位置，其他动作立即向两侧让位作为排序预览；只有在放开指针后才更新草稿。
