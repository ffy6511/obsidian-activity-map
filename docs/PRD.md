...
`typedChars` 是与 `editingMs` 并列的独立统计指标：它只记录符合条件的非空白输入数量，不保存输入内容。其正式语义如下：
...
Header Popover 的指标选择器将它标记为 “Chars”。切换指标只改变当前查询的数值维度，保留用户选择的日期或范围、当前路径、路径/文件分组与文件激活身份；界面只显示计数，绝不显示输入内容。

独立的 Unicode 空白字素（空格、制表符、换行和不换行空格等）不计入；文字、中文、Emoji 与组合字符照常按用户感知字符计数。该规则只适用于更新后的新输入，既有数字记录不回填，因为插件从不保存输入文本。
...
桌面端只在可追踪文件页眉提供一个微型环形图入口。hover 或键盘 focus 打开环形图浮层；点击页眉按钮固定或取消固定该浮层。浮层顶部的控制行可在 Settings 中调整，也可在浮层中长按直接调整；指定日模式时它围绕不可移动的日期导航保留左右两个动作区域。移动端依赖点击打开同一入口，不要求 hover。
...
- 顶部一行由六个可调整的既有动作和固定的中心日期导航组成。指定日模式的中心区域始终是“前一天—`YYYY-MM-DD`—后一天”，日期文字可以打开日期选择器；它不属于可调整动作，不能移动或停用；
- 首次使用时，tracking、路径/文件分组和导出海报位于左侧，Locate、指标和日期范围位于右侧。用户可在两侧排序、将动作拖到另一侧，或移至可恢复的停用区；布局不会改写任何动作的语义；
- 分组切换默认按路径层级聚合，切换后在当前路径范围内递归显示全部现存文件，且不改变指标、时间范围或路径范围；
- “定位当前文件”按钮位于其所属的右侧动作区域，目标始终是 Obsidian 当前激活的工作区文件，即使固定浮层来自先前页面也不会读取旧页眉路径。静止时显示 `locate`，成功定位后与切片、图例行的短暂高亮（约 2 秒）同时平滑过渡为 `locate-fixed`，高亮清除时同时恢复。该高亮等价于同时 hover 切片与图例。图例行不可见时以固定时长（约 400ms）的加速减速动画滚动到该行并尽量居中，目标位于列表边界附近时贴边不溢出；目标已在可视区内时不滚动。按路径分组模式下若文件不在当前范围，先将路径切换到文件所在最近一级目录再高亮。该高亮为临时状态，不打开、选中或固定该文件；
- 左侧交互式环形图，以及位于环形图下方、初始值为 vault 根路径的当前路径；
- 右侧图例列表，按“颜色、名称、精确值、百分比”排列；百分比固定在最右侧；列表最大高度不超过环形图高度，超出后只滚动列表。

路径是独立的上下文信息，不使用列表项背景；默认以次要文字色在环形图下方居中显示。Popover 中当前路径和祖先路径均可点击，hover/focus 时增加下划线并切换为主要文字色。控制行按钮 hover 时均轻微放大，按住时均轻微缩小，系统要求减少动态效果时取消过渡。图例行增加足够的左侧内边距且默认没有背景，名称、精确值和百分比均使用次要色；文件叶子只显示文件名本身，完整 vault 相对路径继续用于身份与打开文件。名称超过可用列宽时以 `…` 截断，不能进入右侧数值区域。hover/focus 某项或 hover/focus 对应饼图切片时，将该行全部文字提升为主要色，并以短缓动降低其余列表项和切片的透明度；离开时使用对应的淡出恢复，系统要求减少动态效果时取消过渡。文件项通过 Obsidian Page Preview 的公开接口提供默认需要 `Cmd/Ctrl` 的原生悬浮预览；预览不打开文件 leaf，指针进入原生预览后继续保留来源 Popover，离开两者后再按原延迟关闭。普通激活仍需可信主键点击或键盘操作。无论按路径还是按文件分组，现存文件切片的首次无修饰键桌面点击仅定位并滚动对应图例行；该切片在指针离开或键盘失焦前显示跳转 cursor，浮层底部以次要色提示再次点击打开页面以及 `Cmd/Ctrl` 新建 tab。`Cmd/Ctrl` 首次点击切片会跳过定位并立即新建 tab；再次点击同一切片会在当前 leaf 打开文件。每次文件打开都会将浮层切换为固定状态，保留其最后的位置，方便继续点击其他文件。文件图例行始终使用跳转 cursor，单次点击即可打开，并支持相同的修饰键行为。目录、“本层文件”、“其他”和已删除项保持原有的一次激活行为；触控仍为单次激活。精确值使用固定宽度右对齐，百分比使用固定宽度在最右侧右对齐，避免秒、分钟或小时格式导致列错位。

分组、指标和时间范围属于查询展示状态，但最后一次成功选择写入插件设置。首次使用默认按路径、Activity 和指定日；关闭后重新打开浮层或重启插件时恢复已保存的 path/file 模式、指标和范围选项。指定日重新打开时始终解析为当天，不会把某个历史日期固化为默认值。按路径模式保留现有目录下钻和“本层文件”语义；按文件模式保留当前 breadcrumb 作为范围，在该范围内将所有现存后代文件扁平显示为独立明细项，并继续使用现有 top-N/“其他”折叠。相同路径、时间范围和指标在两种模式下必须保持相同的 scope total 与 vault total；历史删除数据继续通过不可激活的“已删除”项呈现。控制行布局是独立的本地展示偏好：只有成功保存才会成为下次打开的行，写入失败时当前运行时布局保持不变，草稿仍可重试或取消。

浮层不显示标题栏、重复的指标/总量/vault 占比汇总、图表下方的临时 tooltip 行或普通 tracking 状态页脚。环形图中心总量必须以水平和垂直中心对齐，并使用衬线字体与周围界面文字形成层级。hover 或键盘 focus 切片时只同步高亮切片与现有图例行，不能增删布局节点。打开浮层期间，今天的活动时长每秒投影当前未关闭 session，无需等待 summary 落盘或 30 秒 heartbeat；实时 tick 只原位更新切片几何、中心时长、百分比和列表数值，保留现有 DOM 与 hover/focus 状态。增长最多到最后一次可信活动加空闲阈值，不得将无人操作时间计为活动。点击目录切片在浮层内进入下一级；“本层文件”“其他”“已删除”和文件条目遵循统一的查询激活规则。日期或范围切换保留当前路径。指标选择器的触发按钮只显示随指标变化的语义图标；展开的菜单以文字列出全部指标，触发按钮以可访问名称说明当前选择。所有可点击按钮、切片、图例行和路径在 pointer 环境中使用手型 cursor。
...
控制行只调整 Header Popover 顶部现有的六个动作，不调整环形图、图例、面包屑、状态、固定行为或海报导出 modal。用户有两个入口：

1. 在 Settings 的 `Header popover controls` 中查看完整的一行所见即所得预览。预览包含左右动作区、固定日期导航和下方停用区；它不会实际暂停 tracking、发起查询、下载海报或打开日期选择器。
2. 在 Header Popover 中按住任一已启用的可调整动作 500ms。进入编辑态的这一次按住不会触发原动作，所有可调整动作以轻微抖动提示可拖拽；系统要求减少动态效果时以持续轮廓和焦点提示代替。中心日期导航不抖动，也不是拖放目标。

在两个入口中，拖拽或键盘移动都只改变本地草稿：动作可在任一侧排序、跨越中心日期导航进入另一侧，或拖到下方停用区。停用区中的动作可恢复到任一侧；中心日期导航本身始终固定，也不是拖放目标。键盘操作提供等价的开始移动、区域内调整、跨区域移动、停用、恢复和状态播报。

可拖拽的动作静止时显示 `grab` 光标；按下后切换为 `grabbing`，并将包含原图标或文字的按钮视觉副本随指针移动，产生直接将按钮拿起的效果。行内显示与按钮同尺寸的虚线圆角槽位：初始表示被拖起的位置，移动后表示当前可落位置。指针经过任一侧两个动作之间时，槽位会插入对应位置，其他动作立即向两侧让位作为排序预览；松开时可将动作留在该侧，只有在放开指针后才更新草稿。Popover 中的长按会直接将被按住的动作抬起进入这条拖拽路径，无需再次按下。

Popover 编辑态只替换控制行本身，图表、图例、面包屑和已显示的状态继续可见并保持已挂载。停用动作显示在结果内容下方一行的左侧，`Cancel` 和主操作 `Save` 固定在该行右侧。`Save` 是唯一的持久化路径；保存期间禁用编辑控件，失败时保留草稿并显示可重试错误。`Cancel`、`Escape`、关闭 Popover、结构性重绘或另一处成功保存的新布局都会丢弃本地草稿并恢复最后一次成功保存的行。

海报导出动作可以被排序或停用，其按钮仍只负责打开既有的海报导出 modal。该 modal 的预览、说明文字、下载、格式、文件名和保存位置均不提供布局定制。

### 海报导出 modal

点击导出图标会打开一个独立 modal，不出现二级确认 modal。modal 的主区域是较大的完整 `Wide` 海报预览，导出使用与预览相同的已冻结查询数据和图表模型，不截取 Obsidian 页面或 Popover DOM。当前界面只提供 `Download` 和 `Cancel`：`Download` 自动开始一次本地 PNG 下载，浏览器负责默认 Downloads 目录；modal 不显示输出路径、文件名编辑、排版选择或格式选择。

预览底部正中提供可选说明文字，默认为空。它是无边框、虚线下划线的系统衬线 textarea，可直接点击输入；文字会适度放大、限制最大宽度并最多显示三行。开始编辑时，输入框右侧显示当前视觉行数，例如空文字为 `(0/3)`；超过三行时提示变为错误色。导出 SVG 使用同一文字层，第三行后的内容以省略号收束，避免说明文字与图表重叠。

Wide 海报在左上显示 Activity Map wordmark、右上显示指标和日期/范围；中间复刻 Popover 的语义结构：左侧环形图及其下方路径、右侧最多七项的颜色/名称/精确值/百分比列表。页脚 `Activity Map · Local-first activity insight` 位于右下角。所有颜色从打开 modal 时的 Obsidian 主题解析后内联到 SVG；PNG 从这同一 SVG 以 Retina 密度光栅化。wordmark 当前使用随插件打包的 PNG，未来替换为 SVG 不改变此输出边界。所有用户输入和路径文字必须被转义；导出不得包含笔记正文、正常编辑器中的实际输入文字、选区、剪贴板数据或截图。
...
环形图提供占比、hover、点击和图例联动，并针对目录下钻增加以下约束：
...
页眉入口采用低干扰的图标与可交互浮层。Activity Map 使用 Obsidian 主题变量、标准图标尺寸和组件间距，不依赖固定颜色、字体或外部 DOM 实现。

## 海报导出

当前 Header Popover 的可见导出流程是一个主题匹配的 Wide PNG。运行时按钮、tooltip、hover 高亮等控件不进入文件；浏览器自动使用安全生成的文件名和默认下载位置。导出器仍以独立 SVG 为单一数据源，内部序列化的其他布局或格式不是当前 modal 的产品承诺。
...
| 环形图最大独立切片数 | 8 |
| Header Popover 控制行 | tracking、分组、海报导出｜固定日期导航｜Locate、指标、日期范围，全部启用 |
| 网络与遥测 | 关闭且首版不提供开启项 |

### 用户控制

当前版本提供设置中的排除路径、时间阈值和 Header Popover 控制行所见即所得编辑，以及浮层中的暂停和恢复。控制行编辑保存前不影响运行时，取消或离开设置会丢弃草稿。原始 JSON、SVG、重建和删除控制不在 header-only 界面中暴露，后续页眉数据 modal 必须在破坏性操作前显示范围、不可恢复性和明确确认。
...
- 控制行编辑提供键盘移动和可访问的结果播报；抖动编辑提示遵从 `prefers-reduced-motion`，并保留非动态的轮廓和焦点提示。
...
- Settings 与 Header Popover 的控制行编辑使用相同的已保存布局；自动化和实机验证覆盖固定日期导航、同侧排序、可恢复停用、保存失败、取消与关闭回退，以及编辑期间图表和图例保持可见。
- 键盘能够访问全部核心操作，文本与颜色对比通过自动检查和人工抽查。

### 海报导出与隐私

- Header Popover 可将冻结查询下载为主题匹配的 Wide PNG；它的 SVG 源能由标准解析器打开，且包含可访问标题与描述。
