...
日常入口常驻但不打断写作。当前交互集中在文件页眉的环形图浮层；导出、重建和删除将在后续以显式页眉数据 modal 提供，避免恢复一个常驻侧栏视图。

### 本地可控

用户当前能够检查自己的数据；导出、重建和清除将在后续页眉数据 modal 中提供。数据保留、排除规则和时间阈值均有明确默认值。
...
桌面端只在可追踪文件页眉提供一个微型环形图入口。hover 或键盘 focus 打开环形图浮层；点击页眉按钮固定或取消固定该浮层。控制行最右侧的暂停/恢复图标控制 tracking。移动端依赖点击打开同一入口，不要求 hover。
...
当前版本不注册可停靠完整视图，也不提供命令面板入口。后续 Spec 将以显式页眉数据 modal 承载原始 JSON、SVG、汇总重建和删除控制；该 modal 必须复用浮层的查询状态、`ChartModel`、切片颜色、图例和激活规则，并在破坏性操作前显示范围、不可恢复性和明确确认。
...
当前 header-only 界面不提供 SVG 下载。导出器保留为本地服务能力，后续页眉数据 modal 将针对当前查询提供两类 SVG：
...
- 桌面端承担完整追踪、空闲恢复、页眉浮层和统计能力；导出、重建和删除等待后续页眉数据 modal。
- 移动端至少能够加载数据、切换范围、下钻和查看明细；移动端采集与后续数据 modal 能力按可获得的前台、输入和文件 API 逐项验证。
...
- 后续页眉数据 modal 提供的完整信息图和仅图表 SVG 均能由标准 SVG 解析器打开，且包含可访问标题与描述。
