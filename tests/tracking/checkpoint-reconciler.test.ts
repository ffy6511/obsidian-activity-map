...
describe('checkpoint reconciliation', () => {
	...
	it('clips a stale active checkpoint at its last trusted sample and waits for resume', () => {
		const h = makeEngine();
		const checkpoint: RuntimeCheckpoint = {
			schemaVersion: 1,
			state: 'active',
			currentTarget: { fileId: 'f', path: 'a.md', windowId: 'main', leafId: 'l1' },
			sessionStartedAt: new Date(0).toISOString(),
			lastTrustedActivityAt: new Date(5_000).toISOString(),
			editBurst: null,
			pendingRecovery: [],
			recentDecisions: [],
			savedAt: new Date(5_000).toISOString(),
		};
		const result = reconcileCheckpoint({
			checkpoint,
			engine: h.engine,
			clock: { now: () => sample(200_000) },
			nowSample: sample(200_000),
			currentTarget: checkpoint.currentTarget,
	...
	it('safely closes a checkpoint whose foreground target no longer matches', () => {
		const h = makeEngine();
		const checkpoint: RuntimeCheckpoint = {
			schemaVersion: 1,
			state: 'active',
		...
		reconcileCheckpoint({
			checkpoint,
			engine: h.engine,
			clock: { now: () => sample(10_000) },
			nowSample: sample(10_000),
			currentTarget: { fileId: 'other', path: 'b.md', windowId: 'main', leafId: 'l2' },
			idleThresholdMs: 180_000,
		});
		expect(h.engine.getState()).toBe('untrackable');
		expect(h.sessions[0]?.endedAt).toBe(new Date(5_000).toISOString());
		expect(h.engine.pendingRecovery()).toHaveLength(0);
	});
