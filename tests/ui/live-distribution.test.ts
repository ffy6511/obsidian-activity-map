...
import { buildChartItems, type DistributionItem, type DistributionResult } from '../../src/query/distribution-query';
...
describe('live distribution projection', () => {
	...
	it('keeps a live-only file inside the configured top-N/Other chart bound', () => {
		for (const persistedCount of [8, 9]) {
			const projected = withLiveActivity(populatedFileDistribution(persistedCount), snapshot(), {
				nowMs: Date.parse('2026-07-21T10:00:20.000Z'),
				idleThresholdMs: 180_000,
				timeZone: 'UTC',
			});
			...
			expect(projected.chartItems.reduce((sum, item) => sum + item.value, 0)).toBe(projected.scopeTotal);
		}
	});

	it('repartitions a live file that was already folded into Other', () => {
		const base = populatedFileDistribution(9);
		expect(base.chartItems.find((item) => item.kind === 'other')?.memberIds).toContain('file-8');
