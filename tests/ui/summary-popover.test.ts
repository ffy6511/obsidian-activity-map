...
describe('summary popover fixed layout', () => {
	it('keeps controls, chart, path, and scroll legend without duplicate rows', async () => {
		...
		const leadingActions = source.slice(source.indexOf('leadingActions:'), source.indexOf('],', source.indexOf('leadingActions:')));
		expect(leadingActions.indexOf('this.trackingAction(model)')).toBeLessThan(leadingActions.indexOf('this.groupingAction(model)'));
		expect(leadingActions.indexOf('this.groupingAction(model)')).toBeLessThan(leadingActions.indexOf('this.posterExportAction(model)'));
		...
		expect(source.includes('this.controlsView?.updateAction(this.groupingAction(model))')).toBeTrue();
		...
		expect(source.includes('this.controlsView?.updateAction(this.trackingAction(model))')).toBeTrue();
		...
		for (const expected of ['activity-map-control-button', 'activity-map-icon-button', 'activity-map-day-navigation', 'activity-map-control-actions', 'activity-map-control-leading', 'leadingActions', 'activity-map-query-controls', 'activity-map-dropdown-menu', "role: 'listbox'", 'aria-pressed', 'iconForMetric']) {
		...
		expect(css.includes('grid-template-columns: minmax(0, 1fr) max-content minmax(0, 1fr)')).toBeTrue();
		...
		expect(css.includes('grid-template-columns: 0.75rem minmax(0, 1fr) minmax(7ch, max-content) 4ch')).toBeTrue();
