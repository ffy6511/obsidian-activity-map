...
describe('file-header status presentation', () => {
	...
	it('prioritizes degraded and pending states before ordinary lifecycle state', () => {
		expect(statusPresentation(snapshot({ state: 'degraded', degradedReason: 'write failed' }), 'a.md').className).toBe('is-degraded');
		expect(statusPresentation(snapshot({ pendingRecovery: [{
			candidateId: 'c1', fileId: 'a', pathAtEvent: 'a.md', startedAt: '2026-07-21T00:00:00Z',
			endedAt: '2026-07-21T00:01:00Z', gapMs: 60_000, defaultDecision: 'exclude',
		}] }), 'a.md').className).toBe('is-pending');
	});
	...
	it('maps starting, paused, and idle to named non-color states', () => {
		...
		expect(statusPresentation(snapshot({ state: 'paused' }), 'a.md').className).toBe('is-paused');
