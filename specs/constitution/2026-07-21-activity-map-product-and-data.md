...
- Keep the header popover structurally stable while a slice is highlighted: controls stay above a centered `1:1` result grid with equal inline padding; its left column contains a tightly bounded donut and current path, and its right column contains the bounded scrollable legend. The legend cannot exceed the rendered donut height. Do not add region headings, a transient tooltip row, duplicate totals, vault-share summary, title bar, or ordinary tracking-status footer.
