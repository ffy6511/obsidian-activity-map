...
- Present the current path as centered muted context directly below the donut, outside the legend. Legend rows remain muted by default; pointer or keyboard focus promotes only the active item name, without shifting layout.
