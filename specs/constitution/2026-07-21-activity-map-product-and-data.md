...
Activity Map will be a local-first, cross-platform Obsidian plugin that records trustworthy foreground activity per file, preserves durable file identity and event-time paths, stores settings separately from sharded time-series data, and presents directory aggregates through a progressively disclosed UI. It also records a privacy-preserving `typedChars` count from eligible human text input, keeps the Header Popover's six built-in action controls in a locally persisted two-region layout, and exports the current distribution as an editable, local poster. Desktop receives the complete experience; mobile remains a supported viewer and uses only capabilities verified on that platform.
...
- Persist the last successful layout of the six existing Header Popover actions as a separate small validated setting. The built-in registry owns each action's identity, handler, default placement, and no user-supplied behavior. The preference can set an action's enabled state, region, and region-local order around the fixed date navigation. Invalid or incomplete persisted data normalizes to one safe canonical item for every known action, and a failed save cannot change the committed row.
...
- The Header Popover action row has left and right action regions around fixed selected-day navigation. Its first-run layout keeps tracking, grouping, and poster export on the left, and Locate, metric, and date range on the right. Users may reorder actions within either region, move an action between them, or recoverably disable it; previous day, selected date, and next day are never movable or disableable.
...
- The Header Popover layout contains each known built-in action exactly once. Persisted data can never create a handler, move the fixed date navigation, or remove the recovery path for a disabled action.
...
| 2026-07-28 | Made the Header Popover's six built-in action controls a local persisted, two-region layout preference. | Support reorder, cross-region movement, and recoverable disablement through Settings or direct Popover editing while keeping the selected-day navigation and poster-export dialog contract fixed. |
