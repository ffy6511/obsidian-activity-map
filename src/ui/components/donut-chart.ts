...
export function renderDonutChart(args: {
	...
	for (const item of model.items) {
		...
		path.addEventListener('pointerenter', () => { highlight(item); args.onHighlight?.(item); });
		path.addEventListener('pointerleave', () => { highlight(null); args.onHighlight?.(null); });
		path.addEventListener('focus', () => { highlight(item); args.onHighlight?.(item); });
		path.addEventListener('blur', () => { highlight(null); args.onHighlight?.(null); });
		path.addEventListener('click', () => { highlight(item); args.onHighlight?.(item); args.onActivate(item); });
