...
The repository currently contains the product baseline, an initialized spec-driven delivery workspace, and a minimal buildable Obsidian plugin foundation. No feature Active Spec has started. The plugin registers an Activity Map view, Ribbon action, and command. Activity tracking, persistence, aggregation, charts, settings, and export are not implemented yet.
