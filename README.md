...
- **Header summary** — Open a compact, pinnable activity chart from eligible file headers. In file grouping, a slice first locates its row, then opens on the next click; Cmd/Ctrl-click opens a new tab.
...
- **页眉统计入口**：从符合条件的文件页眉打开可固定的紧凑活动图表；文件分组下切片首次点击定位列表行，再次点击打开文件，`Cmd/Ctrl` 点击新建 tab。
