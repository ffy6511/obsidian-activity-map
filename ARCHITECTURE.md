...
The Header Popover exposes one transient locate action in the query control row, left of the metric dropdown. It drives the same bidirectional chart/legend highlight path as pointer hover for Obsidian's current workspace file. `HeaderActionManager` reads `Workspace.getActiveFile()` when Locate activates and refreshes the control on active-leaf changes, so a fixed Popover never reuses the path of the header that originally opened it. In path grouping, locate dispatches a normal `set-path` to the file's parent directory and completes the highlight on the next ready model of a newer generation; in file grouping it highlights immediately. A single cancelable timer owns the highlight window; re-activation restarts it and re-render/close cancel it. When the legend row is outside the visible list, a fixed-duration ease-in-out centering scroll (`scroll-into-view-animated.ts`) brings it into view instead of the native instant `scrollIntoView`, because the native smooth duration and easing are engine-defined and inconsistent across desktop/mobile; the in-flight animation is cancelled alongside the highlight on re-activation, re-render, and close. Locate never opens, selects, pins, or activates the file.
